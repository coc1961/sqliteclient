...
func parse_escape_sequence(buf *[]byte) (size int, event KeyEvent) {
	...
	if strings.Contains(bufstr, "[1;") {
		switch {
		case strings.Contains(bufstr, "[1;2"):
			//Shift
			shift = true
			b := (*buf)[0:2]
			b = append(b, (*buf)[5:]...)
			*buf = b
			bufstr = string(*buf)
		case strings.Contains(bufstr, "[1;3"):
			//ALT
			alt = true
			b := (*buf)[0:2]
			b = append(b, (*buf)[5:]...)
			*buf = b
			bufstr = string(*buf)
		case strings.Contains(bufstr, "[1;4"):
			//CTRL-ALT
			alt = true
			ctrl = true
			b := (*buf)[0:2]
			...
			*buf = b
			bufstr = string(*buf)
		case strings.Contains(bufstr, "[1;5"):
			//CTRL
			ctrl = true
			b := (*buf)[0:2]
			b = append(b, (*buf)[5:]...)
			*buf = b
			bufstr = string(*buf)
		case strings.Contains(bufstr, "[1;6"):
			//CTRL - SHIFT
			ctrl = true
			shift = true
			b := (*buf)[0:2]
...
func extract_event(inbuf *[]byte) (int, KeyEvent) {
	if len(*inbuf) == 0 {
		return 0, KeyEvent{Data: *inbuf}
	}

	if (*inbuf)[0] == '\033' {
		if len(*inbuf) == 1 {
			return 1, KeyEvent{Key: KeyEsc, Data: *inbuf}
		}
		...
		if size, event := parse_escape_sequence(inbuf); size != 0 {
			...
			return i, KeyEvent{Key: KeyEsc, Err: errors.New("Unrecognized escape sequence"), Data: *inbuf}
		}
	}
	...
	if Key((*inbuf)[0]) <= KeySpace || Key((*inbuf)[0]) == KeyBackspace2 {
		return 1, KeyEvent{Key: Key((*inbuf)[0]), Data: *inbuf}
	}

	// the only possible option is utf8 rune
	if r, n := utf8.DecodeRune(*inbuf); r != utf8.RuneError {
		return n, KeyEvent{Rune: r, Data: *inbuf}
	}
	...
	for {
		...
		case ev := <-input_buf:
			if ev.err != nil {
				...
				case <-quitEvProd:
					return
				case inputComm <- KeyEvent{Err: ev.err, Data: ev.data}:
				}
				break
			}
...
func releaseConsole() {
	quitConsole <- true
	quitEvProd <- true
	_ = unix.IoctlSetTermios(int(out.Fd()), ioctl_SETATTR, &orig_tios)
	out.Close()
	unix.Close(in)
}
